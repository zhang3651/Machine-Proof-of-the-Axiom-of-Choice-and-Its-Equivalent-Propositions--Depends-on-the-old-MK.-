...
Proof.
  ...
  double H; apply Lemma_y with (B:= y∈x) in H2; auto.
  ...
  - assert ((En_S P) ⊂ W).
    ...
    + assert (x ∈ (En_S P) <-> (Ensemble x /\ x ∈ W /\ ~ (P x))).
      { unfold En_S; split; intros.
        ...
        - apply AxiomII; auto. }
      apply Lemma_z in H12; auto; clear H11.
...
Proof.
  ...
  set (P' := (fun n => (forall j : Class, j ∈ n -> P j))).
  ...
  - destruct H2; apply H0; split; intros.
    + apply Theorem134 in H2.
      ...
      unfold W; apply AxiomII; Ens.
    + apply H4.
      unfold PlusOne in H3; apply AxiomII in H3; destruct H3, H6.
      * unfold W in H2; apply AxiomII in H2; destruct H2.
        unfold Integer in H7; destruct H7.
        unfold Ordinal in H7; destruct H7.
        unfold full in H9; eapply H9; eauto.
      * apply AxiomII in H6; destruct H6.
        AssE k; apply Theorem19 in H8; apply H7 in H8.
        subst j; auto.
Qed.
